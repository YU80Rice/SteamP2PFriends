...
1. On both endpoints, set `[Debug] VerboseDiagnostics=false` and `RouteDiagnostics=false`, then select `START` while Unturned is closed. This records the deployed plugin SHA-256 and immutable CFG copy. `v0.2.3.63-beta.2` still records its bounded `[P2P-Connection]` events with both options off.
...
Use a fresh Case ID for each route and direction. The current release is `v0.2.3.63-beta.2`; its SHA-256 is recorded in the matching GitHub Release and audit report after the Release build.
